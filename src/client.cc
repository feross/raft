...
bool send_command(const char * command) {
    ...
    send_loop: while (retries > 0) {
        ...
        if (retries < MAX_CLIENT_RETRIES) {
            ...
            active_server_info = server_infos[rand() % server_infos.size()];
        }
        retries--;
        ...
        leader_info.sin_addr.s_addr = inet_addr(active_server_info.ip_addr.c_str());
        leader_info.sin_port = htons(active_server_info.port);
        ...
        info("Connect to server %s:%d (from %s:%d)",
            active_server_info.ip_addr.c_str(), active_server_info.port,
        ...
        if (redirect_to_leader) {
            active_server_info = *(ServerInfo *) buf;
