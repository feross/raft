...
class Peer {
  public:
    /**
     * Creates & maintains a connection to a peer machine, enabling us to send & receive messages of any format with this peer.
     * Messages are not guarenteed to reach the peer if the peer is down, and will fail silently.
     *
     * @param listening_port - port we will listen for connections from this machine.  This port number must not be shared with anything else because it uniquely identifies the peer.
     * @param destination_ip_address - ip address of the machine we want to peer with
     * @param destination_port - port the peer machine will be listening for our connection on.  Must not be shared on destination machine because it uniquely identifies our machine to the peer.
     * @param message_received_callback - callback function to be called whenever we receive a message
     *              callback arguments:
